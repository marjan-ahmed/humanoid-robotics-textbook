...
MAX_CONTENT_LENGTH = 50000  # Maximum content length to process per page
...
def get_all_urls(base_url: str) -> List[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Sitemap request failed with status: {response.status_code}")
    except Exception as e:
        logger.warning(f"Could not fetch sitemap: {e}")

    # If sitemap didn't work or returned no URLs, try crawling
    if not urls:
        logger.info("Sitemap not found or empty, attempting to crawl...")
        try:
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
            }
            ...
            logger.error(f"Could not crawl base URL: {e}")
...
def extract_text_from_url(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Limit content length to prevent processing very large documents
        if len(text_content) > MAX_CONTENT_LENGTH:
            logger.info(f"Truncating content from {url} from {len(text_content)} to {MAX_CONTENT_LENGTH} characters")
...
def chunk(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = OVERLAP_SIZE) -> List[str]:
    ...
    while start < len(text):
        ...
        # Use simple striding to avoid infinite loops at end of text
    ...
    chunks = [chunk_item for chunk_item in chunks if chunk_item.strip()]
...
def embed(text_chunks: List[str]) -> List[List[float]]:
    ...
    for i in range(0, len(text_chunks), batch_size):
        ...
        logger.info(f"Processing embedding batch {i//batch_size + 1}/{(len(text_chunks)-1)//batch_size + 1}")
...
def main():
    ...
    # Step 0: Create Qdrant collection if it doesn't exist
            ...
            for j, (chunk_text, embedding) in enumerate(zip(text_chunks, embeddings)):
                ...
                if success:
                    ...
                    if j % 10 == 0:  # Log progress every 10 chunks
                        logger.info(f"    Saved chunk {j+1}/{len(text_chunks)}")
                else:
                    logger.error(f"    Failed to save chunk {j}")

        processed_urls += 1

        # Add a delay to be respectful to the target website
        time.sleep(REQUEST_DELAY)

    logger.info(f"\nPipeline completed!")
    logger.info(f"Processed {processed_urls} URLs")
    logger.info(f"Saved {total_chunks} chunks to Qdrant collection '{COLLECTION_NAME}'")
